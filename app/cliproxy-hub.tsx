...
export default function CLIProxyHubScreen() {
  ...
  return (
    <>
      ...
      <ScreenShell title="中心信息" subtitle="CLIProxyAPI 实例和 Mate 客户端信息" safeAreaEdges={['bottom']} bottomInsetClassName="pb-10" refreshing={configQuery.isRefetching || pluginsQuery.isRefetching} onRefresh={async () => { await Promise.all([configQuery.refetch(), pluginsQuery.refetch()]); }}>
        ...
        <AdminSection title="版本信息" detail="上游版本查询由当前 CLIProxyAPI 实例执行。">
          <View className="flex-row items-center gap-3 rounded-2xl bg-[#F6F8FC] p-3 dark:bg-[#152033]"><Info size={20} color="#2F6DF6" /><View className="flex-1"><Text className="text-xs font-bold text-[#172033] dark:text-[#F4F7FB]">Sub2API Mate {Constants.expoConfig?.version || '1.7.1'}</Text><Text className="mt-1 text-[10px] text-[#7B8798] dark:text-[#9EABC0]">CLIProxy 独立工作区</Text></View></View>
