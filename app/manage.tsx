...
const modules = [
  { title: '代理管理', detail: '维护出口代理，执行连通性与质量检测', route: '/proxies', icon: Network },
  { title: '兑换码', detail: '批量生成余额、并发、订阅和邀请码', route: '/redeem', icon: Ticket },
  { title: '错误中心', detail: '查看请求错误与上游错误并标记处理状态', route: '/ops-errors', icon: AlertTriangle },
  { title: '订阅管理', detail: '分配、延期、重置额度和撤销用户订阅', route: '/subscriptions', icon: UsersRound },
  { title: '渠道管理', detail: '维护可用渠道与分组绑定', route: '/channels', icon: Waypoints },
  { title: '风控中心', detail: '配置内容审核并查看命中记录', route: '/risk-control', icon: ShieldCheck },
  { title: '系统设置', detail: '管理注册、默认额度和业务功能开关', route: '/system-settings', icon: Settings2 },
  { title: '合规确认', detail: '查看当前部署合规状态并完成确认', route: '/compliance', icon: ClipboardCheck },
  { title: '操作审计', detail: '追踪管理端变更、操作者和请求结果', route: '/audit-logs', icon: ScrollText },
  { title: '使用记录', detail: '查看模型调用、Token、费用与响应耗时', route: '/usage-logs', icon: Activity },
  { title: '公告管理', detail: '创建、发布、归档和删除用户公告', route: '/announcements', icon: Megaphone },
  { title: '优惠码', detail: '创建奖励码并管理使用次数与启停状态', route: '/promo-codes', icon: BadgePercent },
  { title: '提示词审计', detail: '管理审计策略、运行状态和风险事件', route: '/prompt-audit', icon: ScanSearch },
  { title: '备份与数据', detail: '数据库快照、恢复、计划任务和备份代理', route: '/backups', icon: DatabaseBackup },
  { title: '系统维护', detail: '检查更新、升级、回滚和重启服务', route: '/system-maintenance', icon: Wrench },
  { title: '用户属性', detail: '维护自定义用户字段、启停与显示顺序', route: '/user-attributes', icon: ListPlus },
  { title: '流量策略', detail: '错误透传规则与 TLS 指纹配置', route: '/traffic-policies', icon: Route },
  { title: '定时测试', detail: '为账号创建定时连通性测试计划', route: '/scheduled-tests', icon: Timer },
  { title: '渠道监控', detail: '定时探测外部渠道并查看可用率', route: '/channel-monitors', icon: RadioTower },
  { title: '推广返利', detail: '管理推广用户、邀请、返利与划转记录', route: '/affiliates', icon: Share2 },
  { title: '运维中心', detail: '处理告警事件并检索结构化系统日志', route: '/ops-center', icon: Siren },
  { title: 'OAuth 授权', detail: '完成各平台账号授权码与 Token 交换', route: '/oauth-center', icon: KeyRound },
  { title: '构建与同步', detail: '手机触发 EAS 构建、下载 APK 和同步上游', route: '/build-center', icon: Blocks },
  { title: 'AI 助手', detail: '独立对话、接口诊断并生成受控修复 PR', route: '/ai-assistant', icon: MessageCircle },
  { title: 'GitHub 配置', detail: '保存目标仓库、基础分支和 GitHub Token', route: '/github-settings', icon: Github },
  { title: '全部 API', detail: '检索并调用全部 Sub2API 管理端接口', route: '/api-console', icon: Braces },
] as const;
...
export default function ManageScreen() {
  ...
  const role = config.user?.role === 'admin' || config.authMode === 'admin_key' ? '管理员' : '普通用户';
  ...
  return (
    <>
      ...
      <ScreenShell title="更多管理" subtitle="当前登录、语言与 Sub2API 高级管理功能" bottomInsetClassName="pb-8" safeAreaEdges={['bottom']}>
        ...
        {modules.map(({ title, detail, route, icon: Icon }) => (
          <Pressable
            key={route}
