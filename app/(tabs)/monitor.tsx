...
export default function MonitorScreen() {
  ...
  return <SafeAreaView style={{ flex: 1, backgroundColor: dark ? '#0B1220' : '#F4F7FC' }}><ScrollView contentContainerStyle={{ padding: 16, paddingTop: 70, paddingBottom: 50, gap: 12 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#2F6DF6" />}>
    <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}><View style={{ flex: 1 }}><Text style={{ fontSize: 20, fontWeight: '800', color: dark ? '#F4F7FB' : '#172033' }}>仪表盘</Text><Text numberOfLines={1} style={{ marginTop: 4, fontSize: 11, color: dark ? '#9EABC0' : '#758297' }}>{config.user?.email || config.baseUrl} · {admin ? '管理员' : '普通用户'}</Text></View><View style={{ flexDirection: 'row', gap: 5 }}>{(['24h', '7d', '30d'] as Range[]).map((value) => <Pressable key={value} onPress={() => setRange(value)} style={{ borderRadius: 999, paddingHorizontal: 10, paddingVertical: 7, backgroundColor: range === value ? '#2F6DF6' : dark ? '#273449' : '#E5EBF4' }}><Text style={{ color: range === value ? '#fff' : dark ? '#D5DDEA' : '#607086', fontSize: 10, fontWeight: '800' }}>{value.toUpperCase()}</Text></Pressable>)}</View></View>
