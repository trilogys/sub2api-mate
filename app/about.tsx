    ...
    <>
      ...
      <ScreenShell title="关于应用" subtitle="版本、在线更新与开源信息" bottomInsetClassName="pb-8" refreshing={releaseQuery.isRefetching} onRefresh={() => releaseQuery.refetch().then(() => undefined)}>
        ...
        <AdminSection title="升级提示" detail="自动检查 GitHub Release；下拉页面也可以重新检查。">
          ...
          {hasUpdate ? (
            <AdminButton
              label={Platform.OS === 'android' && apk ? `下载 ${apk.name}` : '打开 Release 页面'}
              onPress={() => void open(Platform.OS === 'android' && apk ? apk.browser_download_url : release!.html_url)}
            />
          ) : null}
          <AdminMessage error={releaseQuery.error} />
          <Text className="text-[10px] leading-4 text-[#7B8798] dark:text-[#9EABC0]">APK 可以直接下载，但 Android 会要求你确认安装；签名不一致时需要先卸载旧版。App 无法静默覆盖安装。</Text>
        </AdminSection>
